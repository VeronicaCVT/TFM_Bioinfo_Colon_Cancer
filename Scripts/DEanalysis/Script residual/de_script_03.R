...
output_dir <- file.path("results", dataset_name)
...
data <- data_raw[3:17] %>%
  select(PN1, PN2, PN3, PN4, PN5, PA1, PA2, PA3, PA4, PA5, PT1, PT2, PT3, PT4, PT5)
head(data)
dim(data)
...
# Subdata
data_subset <- data %>% 
  select(PN1, PN2, PN3, PN4, PN5, PT1, PT2, PT3, PT4, PT5)
dim(data_subset)
...
pca_plot500 <- plotPCA(rld_sin_PA, intgroup = c("Condition"), ntop=500) + 
     ...
     labs(title = "PCA - Comparación de Condiciones")
pca_plot500
...
top1500_sig_genes <- rownames(resLFC_ordered)[1:1500]
...
plotMA(resLFC, 
       ...
       cex = 0.4)  
abline(h = c(-0.585, 0.585), col = "red", lwd = 2, lty = 2) 
dev.off()
...
df_anotacion <- as.data.frame(colData(dds))[, c("Condition", "Patient")]
muestras_interes <- rownames(df_anotacion[df_anotacion$Condition %in% c("PN", "PT"), ])

# Extraer matriz rlog solo de genes top y muestras de interés
mat_filtrada <- assay(rld)[top20_sig_genes, muestras_interes]
df_filtrado <- df_anotacion[muestras_interes, , drop=FALSE]

# Guardar Heatmap (pheatmap tiene argumento filename integrado)
pheatmap(mat_filtrada, 
         cluster_rows = TRUE, # Cambiado a TRUE para agrupar genes con perfiles similares
         show_rownames = TRUE,
         cluster_cols = TRUE,    
...
# Añadir una columna con los nombres de los genes para las etiquetas
volcano_data$gene <- rownames(volcano_data)

# Filtrar los datos para aislar solo los Top 20 genes (para etiquetarlos en el gráfico)
top_genes_data <- volcano_data %>% filter(gene %in% top20_sig_genes)
...
volcano_plot <- ggplot(volcano_data, aes(x = log2FoldChange, y = -log10(padj), color = Significance)) +
  ...
  # Añadir líneas de corte (umbrales)
  geom_vline(xintercept = c(-0.585, 0.585), col = "black", linetype = "dashed", alpha = 0.5) +
  geom_hline(yintercept = -log10(0.05), col = "black", linetype = "dashed", alpha = 0.5) +
  # Añadir los nombres de los Top 20 genes evitando que se solapen
  geom_text_repel(data = top_genes_data, aes(label = gene), 
                  size = 3, color = "black", box.padding = 0.5, max.overlaps = 20) +
  theme_light() +
  labs(title = "Volcano Plot: Tumor (PT) vs Normal (PN)",
       x = "Log2 Fold Change",
       y = "-Log10(Adjusted P-value)",
       color = "Estado de Expresión") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold"),
        legend.position = "bottom")

volcano_plot

ggsave(file.path(output_dir, "Volcano_Plot.png"), plot = volcano_plot, width = 9, height = 7, dpi = 300)
